...
function [volts, I_LED, I_det, EQE, centre_wavelength, EQE_approx] = ...
    measure_EQE(src, phtd, integration_time, max_voltage, step_size, responsivity, wavelengths, EL_Spec, ...
...
%   raw_only: set to true to bypass EQE measurements (for debugging)
    ...
    for k=1:length(volts)
        ...
        if ~raw_only
            if size(EL_Spec,1) > 3
                [EQE(k), centre_wavelength(k), EQE_approx(k)] = calculate_EQE_2(wavelengths, EL_Spec(:,1), EL_Spec(:,k+1), I_LED(k), I_det(k), responsivity);
            else
                centre_wavelength(k) = EL_Spec;
                EQE_approx(k) = approximate_EQE(EL_Spec, responsivity, I_det(k), I_LED(k));
                EQE(k) = EQE_approx(k);
            end
        end
        ...
        set(graph_handles(3),'YData', EQE*100);
